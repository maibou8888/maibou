...
#define OpenUDI//Dlog(fmt, ...)
//#define OpenUDI//Dlog(fmt, ...) Dlog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define OpenUDI//Dlog(fmt, ...) Dlog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
    ...
    if (item) {
        @try{
            ...
            OpenUDI//Dlog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        OpenUDI//Dlog(@"localDict = %@",localDict);
    }
...
#endif
        OpenUDI//Dlog(@"SlotPB name = %@",slotPBid);
        if (slotPB==nil) {
    ...
    //
    OpenUDI//Dlog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    OpenUDI//Dlog(@"Local dict after opt-out = %@",dict);
