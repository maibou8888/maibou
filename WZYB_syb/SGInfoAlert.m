...
+ (void)showInfo1:(NSString *)info
         bgColor:(CGColorRef)color
          inView:(UIView *)view 
        vertical:(float)height
{
    SGInfoAlert *sgAlert = [SGInfoAlert new];
    [sgAlert showInfo1:info bgColor:color inView:view vertical:height];
}
...
- (void)showInfo1:(NSString *)info
         ...
         vertical:(float)height
{
    bdkString = [NSString stringWithFormat:@"%@",info];
    if (self.notify.isAnimating) return;
    
    [view addSubview:self.notify];
    [self.notify presentWithDuration:1.0f speed:0.5f inView:view completion:^{
...
- (BDKNotifyHUD *)notify {
    ...
    _notify = [BDKNotifyHUD notifyHUDWithImage:[UIImage imageNamed:@"Checkmark@2x.png"] text:bdkString];
    if (Phone_Height > 480) {
        _notify.center = CGPointMake(160,264);
    }else
    _notify.center = CGPointMake(160,220);
    return _notify;
}
